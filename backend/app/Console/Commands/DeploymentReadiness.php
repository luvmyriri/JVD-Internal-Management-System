...
class DeploymentReadiness extends Command
{
    ...
    public function handle():int
    {
        ...
        Artisan::call('migrate:status');$checks['No pending migrations']=!str_contains(Artisan::output(),'Pending');
        $checks['Storage writable']=is_writable(storage_path('framework'))&&is_writable(storage_path('logs'));
        $checks['PayMongo test/live key configured']=filled(config('services.paymongo.secret_key'));
        ...
        foreach($checks as $label=>$ok)$this->{$ok?'info':'error'}(($ok?'PASS ':'FAIL ').$label);
        $failed=array_filter($checks,fn($ok)=>!$ok);
        $this->newLine();$this->line(count($failed)?count($failed).' readiness checks failed.':'All deployment readiness checks passed.');
        return count($failed)?self::FAILURE:self::SUCCESS;
    }
    private function attempt(callable $callback):bool{try{$callback();return true;}catch(\Throwable){return false;}}
}
