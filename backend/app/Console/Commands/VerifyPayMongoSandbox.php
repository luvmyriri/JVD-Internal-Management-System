...
{
    protected $signature='paymongo:verify-sandbox {--amount=100 : Test amount in PHP}';
    ...
    public function handle(PayMongoService $payMongo):int
    {
        ...
        $amount=max(100,(float)$this->option('amount'));
        $result=$payMongo->createCheckoutSession(['line_items'=>[['amount'=>(int)round($amount*100),'currency'=>'PHP','name'=>'JVD sandbox readiness verification','quantity'=>1]],'description'=>'JVD management system sandbox verification','payment_method_types'=>['gcash','card']]);
