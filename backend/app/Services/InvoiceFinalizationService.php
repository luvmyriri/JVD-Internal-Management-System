        ...
        if ($booking && $booking->travel_date) {
            ...
            if ($booking->bus_id) {
                ...
                $travelConflict = \DB::table('travels')
                    ...
                    ->first();
                if ($travelConflict) {
                    throw new \InvalidArgumentException("Vehicle is already reserved for a travel on " . $dateStr . ".");
                }
            ...
            if ($booking->driver_id) {
                ...
                $driverTravelConflict = \DB::table('travels')
                    ...
                    ->lockForUpdate()
                    ->first();
                if ($driverTravelConflict) {
                    throw new \InvalidArgumentException("Driver is already reserved for a travel on " . $dateStr . ".");
                }
            }
        }
        ...
        if ($hasBusService && $booking && $booking->bus_id) {
            DB::transaction(function () use ($invoice, $booking, $busServiceDate, $busDestination) {
                ...
                TripTicket::create([
                    ...
                    'date_of_travel'   => $busServiceDate ?? $booking->travel_date ?? now()->addDay()->toDateString(),
