...
class SalesOrderService
{
    ...
    public function createDraft(array $data, int $actorId): SalesOrder
    {
        return DB::transaction(function () use ($data, $actorId) {
            ...
            foreach ($data['items'] ?? [] as $item) {
                $this->addItem($order, $item, $actorId);
            }
    ...
    public function markQuoted(SalesOrder $order, int $actorId): SalesOrder
    {
        return DB::transaction(function () use ($order, $actorId) {
            ...
            if ($order->items->isEmpty()) {
                throw ValidationException::withMessages(['items' => 'Add at least one service before issuing a quote.']);
            }
            $from = $order->status;
    ...
    public function confirm(SalesOrder $order, array $payment, int $actorId): SalesOrder
    {
        $invoice = DB::transaction(function () use ($order, $payment, $actorId) {
            ...
            if (! $order->customer_id || ! $order->customer) {
                throw ValidationException::withMessages(['customer_id' => 'Select or create the customer before confirmation.']);
            }
            if ($order->items->isEmpty()) {
                throw ValidationException::withMessages(['items' => 'Add at least one service before confirmation.']);
            }
            foreach ($order->items as $item) {
                if (! $item->service_id || ! $item->fulfillment) {
                    throw ValidationException::withMessages(['items' => "Line {$item->line_number} is incomplete."]);
                }
        ...
        return DB::transaction(function () use ($invoice, $actorId) {
            ...
            foreach ($invoiceItems as $index => $invoiceItem) {
                ...
                if ($invoice->status !== 'cancelled') {
                    app(TripTicketService::class)->synchronizeForSalesItem($orderItem, $actorId ?? $invoice->created_by);
                }
            }
    ...
    private function legacyInvoiceItemIndex($invoiceItems, array $legacy, SalesOrder $order): ?int
    {
        ...
        if ($legacy['type']) {
            foreach ($invoiceItems as $index => $invoiceItem) {
                if ($this->invoiceItemServiceType($invoiceItem) === $legacy['type']) {
                    return $index;
                }
            }
        }
        ...
        foreach ($invoiceItems as $index => $invoiceItem) {
            if (! $this->lineUsesTypedCapture($invoiceItem)) {
                return $index;
            }
        }
    ...
    public function resynchronizeFulfillment(SalesOrderItem $item): void
    {
        ...
        app(TripTicketService::class)->synchronizeForSalesItem($item->fresh(['order.invoice', 'fulfillment', 'service']), $item->order->agent_id);
    }
        ...
        if (in_array($item->service_type, ['private_tour', 'transfer_service'], true)) {
            ...
            if ($busId) {
                ...
                if ($bus->status !== 'available') {
                    throw ValidationException::withMessages(["items.{$item->line_number}.bus_id" => 'Vehicle is not operationally available.']);
                }
                if ($fulfillment->passenger_count > $bus->seating_capacity) {
                    throw ValidationException::withMessages(["items.{$item->line_number}.passenger_count" => 'Passenger count exceeds vehicle capacity.']);
                }
            }
            if ($driverId) {
                $driver = User::lockForUpdate()->findOrFail($driverId);
                if ($driver->role !== 'driver' || ! $driver->is_active) {
                    throw ValidationException::withMessages(["items.{$item->line_number}.driver_id" => 'Driver is not active.']);
                }
            }
            if ($busId || $driverId) {
    ...
    {
        foreach ($order->items()->orderBy('line_number')->get() as $index => $item) {
            $item->update(['line_number' => $index + 1]);
        }
    }
