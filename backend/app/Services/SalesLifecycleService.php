...
class SalesLifecycleService
{
    ...
    public function approve(SalesOrderAdjustment $adjustment, int $actorId): SalesOrderAdjustment
    {
        return DB::transaction(function () use ($adjustment,$actorId) {
            ...
            if ($adjustment->status !== 'pending_approval') throw ValidationException::withMessages(['adjustment'=>'Only pending requests can be approved.']);
    ...
    {
        if ($adjustment->status !== 'pending_approval') throw ValidationException::withMessages(['adjustment'=>'Only pending requests can be rejected.']);
    ...
    {
        if ($creditNote->status !== 'posted') throw ValidationException::withMessages(['credit_note'=>'Credit note must be posted before requesting a refund.']);
        ...
        if ($amount <= 0 || $amount > min($available, $cashAvailable)) throw ValidationException::withMessages(['amount'=>'Refund exceeds the posted credit or collected amount available.']);
    ...
    {
        if ($refund->status !== 'pending_approval') throw ValidationException::withMessages(['refund'=>'Only pending refunds can be approved.']);
    ...
    public function processRefund(SalesRefund $refund, ?string $destinationReference, int $actorId): SalesRefund
    {
        return DB::transaction(function () use ($refund,$destinationReference,$actorId) {
            ...
            if ($refund->status !== 'approved') throw ValidationException::withMessages(['refund'=>'Refund requires approval before processing.']);
    ...
    private function approveCancellation(SalesOrderAdjustment $adjustment, int $actorId): void
    {
        ...
        if ((float) $adjustment->amount > 0) $this->issueAndPostCredit($adjustment, $actorId);
    ...
    {
        if($fulfillment instanceof \App\Models\JoinerReservation){
            $reservation=\App\Models\JoinerReservation::lockForUpdate()->with(['passengers.seat','departure'])->findOrFail($fulfillment->id);
            $released=0;
            foreach($reservation->passengers as $passenger){
                if($passenger->seat){$passenger->seat->update(['status'=>'available','held_until'=>null]);$released++;}
            }
            if($released>0){
                $departure=\App\Models\JoinerDeparture::lockForUpdate()->findOrFail($reservation->departure_id);
        ...
        if(array_key_exists('status',$fulfillment->getAttributes()))$fulfillment->update(['status'=>'cancelled']);
    }

    private function approveAmendment(SalesOrderAdjustment $adjustment, int $actorId): void
    {
        $changes = $adjustment->change_set ?? [];
        if (isset($changes['financial'])) throw ValidationException::withMessages(['change_set.financial'=>'Posted prices require a credit note or a supplemental order; they cannot be overwritten.']);
        foreach ($changes['items'] ?? [] as $change) {
            $item = $adjustment->order->items->firstWhere('id', $change['item_id'] ?? null);
            if (!$item || !$item->fulfillment) throw ValidationException::withMessages(['change_set.items'=>'Amendment references an invalid order line.']);
    ...
    private function issueAndPostCredit(SalesOrderAdjustment $adjustment, int $actorId): CreditNote
    {
        ...
        if ($tax > 0) $lines[] = ['account_id'=>$vat->id,'debit'=>$tax,'credit'=>0,'description'=>'Reverse output VAT'];
        if ($arCredit > 0) $lines[] = ['account_id'=>$ar->id,'debit'=>0,'credit'=>$arCredit,'description'=>'Reduce accounts receivable'];
        if ($refundLiability > 0) $lines[] = ['account_id'=>$payable->id,'debit'=>0,'credit'=>$refundLiability,'description'=>'Customer refund liability'];
