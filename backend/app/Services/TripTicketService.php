...
class TripTicketService
{
    ...
    public function synchronizeForEducationalTourPackage(EducationalTourPackage $package): Collection
    {
        ...
        return DB::transaction(function () use ($package) {
            // Serialize every synchronization for this package. The database
            // unique key below is the final guard; this lock also prevents two
            // checkout/edit side effects from racing to create the same ticket.
            $package = EducationalTourPackage::whereKey($package->id)
                ->lockForUpdate()
                ->firstOrFail();
            $package->load(['busAssignments.bus', 'busAssignments.driver', 'schoolCustomer']);
            ...
            $activeAssignmentIds = [];
            foreach ($package->busAssignments->sortBy('sequence_number')->values() as $index => $assignment) {
                ...
                $candidateTickets = TripTicket::where(function ($query) use ($package, $assignment, $attributes) {
                    $query->where('educational_tour_bus_assignment_id', $assignment->id)
                        ->orWhere(function ($stableKey) use ($package, $attributes) {
                            $stableKey->where('educational_tour_package_id', $package->id)
                                ->where('assignment_index', $attributes['assignment_index']);
                        });
                })
                    ->orderByRaw("CASE status WHEN 'completed' THEN 1 WHEN 'approved' THEN 2 WHEN 'draft' THEN 3 ELSE 4 END")
                    ->orderBy('id')
                    ->lockForUpdate()
                    ->get();

                $ticket = $candidateTickets->first();

                // Repair historical duplicates while preserving the ticket that
                // progressed furthest through approval/completion.
                $superseded = $candidateTickets->skip(1);
                if ($superseded->isNotEmpty()) {
                    WorkOrder::whereIn('trip_ticket_id', $superseded->pluck('id'))
                        ->whereNotIn('status', ['completed', 'cancelled'])
                        ->update(['status' => 'cancelled']);
                    TripTicket::whereIn('id', $superseded->pluck('id'))->update([
                        'status' => 'cancelled',
                ...
                if ($ticket) {
                    if ($ticket->status !== 'completed') {
                        $operationalChanged = collect($attributes)
                            ->except([
                                'issue_date',
                                'educational_tour_package_id',
                                'educational_tour_bus_assignment_id',
                            ])
                            ->contains(fn ($value, $field) => (string) $ticket->{$field} !== (string) $value);
                        ...
                        $ticket->update([
                            ...
                            // An operational edit invalidates a prior approval;
                            // cancelled tickets are revived when their bus slot is restored.
                            'status' => $ticket->status === 'cancelled' || ($ticket->status === 'approved' && $operationalChanged)
                                ? 'draft'
                                : $ticket->status,
                            'approved_by' => $ticket->status === 'approved' && $operationalChanged
                                ? null
                                : $ticket->approved_by,
                        ]);
                    }
        ...
        $result = DB::transaction(function () use ($ticket, $validated, $driverChanged, $busChanged, $dateChanged, $durationChanged, $salesFactsChanged, $newDriverId, $newBusId, $newDate, $newDuration, $request, $user, $salesItem) {
            if ($driverChanged || $busChanged || $dateChanged || $durationChanged) {
                ...
                if ($salesItem) {
                    ...
                    if ($fulfillment instanceof CharterBooking) {
                        ...
                        if (empty($assignments)) {
                            ...
                            if (isset($assignments[$idx])) {
                                $assignments[$idx]['bus_id'] = $newBusId;
                                $assignments[$idx]['driver_id'] = $newDriverId;
                                $assignments[$idx]['plate_number'] = $bus?->plate_number;
                            }
                        }
                    ...
                    } elseif ($fulfillment && in_array(get_class($fulfillment), [PrivateTourBooking::class, TransferBooking::class, Booking::class])) {
                        $fUpdates = [
                            'bus_id' => $newBusId,
                            'driver_id' => $newDriverId,
                        ];
                        if ($dateChanged && isset($validated['date_of_travel'])) {
                            ...
                            $fUpdates['ends_at'] = $travelDate->copy()->endOfDay();
                        }
                        if (isset($validated['pick_up']) && isset($fulfillment->pickup_location)) {
                            $fUpdates['pickup_location'] = $validated['pick_up'];
                        }
                        if (isset($validated['drop_off']) && isset($fulfillment->destination)) {
