...
/**
 * All role-dashboard aggregation logic, extracted from DashboardController (roadmap 2.9)
 * so the controller is a thin HTTP adapter. Behaviour is unchanged — see
 * DashboardEndpointsTest for the characterization contract.
 */
class DashboardService
{
    /** Philippine provinces / destinations treated as LOCAL (case-insensitive keyword matching). */
    private const LOCAL_DESTINATION_KEYWORDS = [
    ...
    public function accountingData(): array
    {
        $now   = Carbon::now();
    ...
    public function driverData(User $user): array
    {
        $now   = Carbon::now();
        $month = $now->copy()->startOfMonth();
    ...
    public function approvalsData(): array
    {
    ...
    private function monthlyChartData(): array
    {
        ...
        foreach ($months as $i => $label) {
            ...
            $result[] = [
                'month'       => $label,
                'revenue'     => $rev ? round($rev->revenue / 1000, 1) : 0,
