...
class CharterController extends Controller
{
    ...
    {
        return response()->json(['data' => CharterRatePlan::with('service:id,name,description')->where('is_active', true)->orderBy('vehicle_class')->orderBy('name')->get()]);
    }
    ...
    public function storeRatePlan(StoreCharterRatePlanRequest $request)
    {
        ...
        if ($service->is_sales_catalog === false) {
            throw \Illuminate\Validation\ValidationException::withMessages(['service_id' => 'Select an active sales catalog service.']);
        }
    ...
    public function resources(Request $request)
    {
        ...
        $startDate = \Carbon\Carbon::parse($data['starts_at'])->toDateString();
        $endDate = \Carbon\Carbon::parse($data['ends_at'])->toDateString();
        $busIds = $busIds
            ->merge(\App\Models\TripTicket::where('status', '!=', 'cancelled')->whereBetween('date_of_travel', [$startDate, $endDate])->pluck('bus_id'))
            ->merge(\App\Models\PmsSchedule::where('status', '!=', 'cancelled')->whereBetween('maintenance_date', [$startDate, $endDate])->pluck('bus_id'))->filter()->unique();
        $driverIds = $driverIds
            ->merge(\App\Models\TripTicket::where('status', '!=', 'cancelled')->whereBetween('date_of_travel', [$startDate, $endDate])->pluck('driver_id'))->filter()->unique();
