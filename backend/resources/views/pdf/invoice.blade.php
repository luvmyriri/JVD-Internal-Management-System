...
<body>
    ...
    <table class="header-table">
        <tr>
            <td class="header-left">
                ...
                <div style="margin-top: 8px; font-size: 9px; color: #475569; line-height: 1.4;">
    ...
    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; margin-bottom: 20px;">
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 10px;">
            <tr>
                @if($b->travel_date)
                <td style="width: 33.3%; padding: 4px 0; vertical-align: top;"><strong>Travel Date:</strong> {{ \Carbon\Carbon::parse($b->travel_date)->format('F d, Y') }}</td>
                @endif
                @if($b->tour_code)
                <td style="width: 33.3%; padding: 4px 0; vertical-align: top;"><strong>Tour/Joiner Code:</strong> {{ $b->tour_code }}</td>
                @endif
                @if($b->pax_count)
                <td style="width: 33.3%; padding: 4px 0; vertical-align: top;"><strong>Pax Count:</strong> {{ $b->pax_count }} Pax</td>
                @endif
            </tr>
            @if($b->pickup_location)
            <tr>
                <td colspan="3" style="padding: 4px 0; vertical-align: top;"><strong>Pickup Location:</strong> {{ $b->pickup_location }}</td>
            </tr>
            @endif
            @if($b->bus_id || $b->driver_id)
            <tr>
                @if($b->bus)
                <td style="padding: 4px 0; vertical-align: top;"><strong>Bus Assigned:</strong> {{ $b->bus->plate_number }} ({{ $b->bus->model }})</td>
                @endif
                @if($b->driver)
                <td style="padding: 4px 0; vertical-align: top;"><strong>Driver:</strong> {{ $b->driver->first_name }} {{ $b->driver->last_name }}</td>
                @endif
                @if($b->seat_map && count($b->seat_map) > 0)
                <td style="padding: 4px 0; vertical-align: top;"><strong>Selected Seats:</strong> {{ implode(', ', $b->seat_map) }}</td>
                @endif
            </tr>
            @endif
        </table>
