...
export function EducationalProgramDetails() {
  ...
  return <div className="mx-auto max-w-[1400px] space-y-5 pb-12">
    ...
    <section className="rounded-2xl border border-border bg-surface"><div className="border-b border-border p-5"><h2 className="text-xl font-black text-ink">Schools, customers, invoices and schedules</h2><p className="mt-1 text-sm text-muted">Open an issued invoice to access its manifest, quotation, contract, payment and DTT handoff.</p></div>{bookings.length === 0 ? <div className="p-10 text-center"><GraduationCap className="mx-auto h-8 w-8 text-muted" /><p className="mt-3 font-black text-ink">No bookings for this program yet</p></div> : <div className="divide-y divide-border">{bookings.map(booking => <button key={booking.id} type="button" disabled={!booking.invoice?.id} onClick={() => booking.invoice?.id && navigate(`/sales/transactions/${booking.invoice.id}`)} className="grid w-full gap-3 p-5 text-left hover:bg-surface-alt disabled:cursor-not-allowed disabled:opacity-60 sm:grid-cols-[1.2fr_1fr_1fr_auto] sm:items-center"><div><p className="font-black text-ink">{booking.school_name}</p><p className="mt-1 text-xs text-muted">{booking.contact_person} · {booking.contact_email || booking.contact_number || 'No contact recorded'}</p></div><div><p className="font-bold text-ink">{booking.invoice?.invoice_number || 'Invoice not issued'}</p><p className="mt-1 text-xs uppercase text-muted">{booking.status}</p></div><div><p className="font-bold text-ink">{dateTime(booking.starts_at)}</p><p className="mt-1 text-xs text-muted">{booking.student_count + (booking.adult_count ?? (booking as any).chaperone_count ?? 0)} travelers</p></div><span className="text-xs font-black text-brand">Open details</span></button>)}</div>}</section>
  </div>;
}
