...
import { ArrowLeft, Bus, CalendarClock, CheckCircle2, ChevronDown, Eye, Fuel, Gauge, LockKeyhole, Plus, Route, ShieldCheck, TriangleAlert, UsersRound, UserRound, Sparkles, Users, Pencil, Trash2 } from 'lucide-react';

import toast from 'react-hot-toast';
import { billingApi, type Service } from '../../api/billing';
import { charterApi, type CharterRatePlan, type RouteEstimate, type TollMatrixCalculation } from '../../api/charters';
        ...
        <main className="mx-auto grid max-w-7xl gap-6 px-5 py-6 lg:grid-cols-[minmax(0,1fr)_340px] lg:px-8">
          <div className="space-y-5">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <label className="mb-4 block text-xs font-bold text-slate-600">Toll pricing method<select value={tollPricingMode} onChange={event => { const mode = event.target.value as 'route' | 'matrix' | 'manual'; setTollPricingMode(mode); if (mode === 'route' && routeEstimate && routeEstimate.toll_estimate.mode !== 'manual_reference') { setTollFeeEst(String(routeEstimate.toll_estimate.toll_gate_fees)); setEasytripEst(String(routeEstimate.toll_estimate.easytrip)); setAutosweepEst(String(routeEstimate.toll_estimate.autosweep)); } }} className={inputClass}><option value="route">Automatic from mapped route</option><option value="matrix">Choose Class 2 matrix entries</option><option value="manual">Manual amounts</option></select></label>
