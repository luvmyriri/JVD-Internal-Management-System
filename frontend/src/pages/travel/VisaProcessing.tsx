...
import {
  ...
  LuCalendar, LuChevronRight, LuCircleCheck, LuCircle,
} from 'react-icons/lu';
...
// ── Kanban Card ────────────────────────────────────────────────────────────────
function VisaCard({ c, onClick }: { c: VisaCase; onClick: () => void }) {
  ...
  return (
    <div
      onClick={onClick}
      className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-4 cursor-pointer hover:shadow-lg hover:border-violet-200 dark:hover:border-violet-800 transition-all"
    >
      ...
      <div className="mb-2">
        <div className="flex justify-between mb-1">
          <span className="text-[9px] text-gray-400 font-bold uppercase">Docs</span>
          <span className="text-[9px] font-bold text-violet-600">{done}/{VISA_CHECKLIST.length}</span>
        </div>
...
export default function VisaProcessing() {
  ...
  const grouped = STATUS_FLOW.reduce<Record<string, VisaCase[]>>((acc, status) => {
          ...
          <div className="flex gap-4 min-w-max">
                ...
                <div className="flex-1 flex flex-col h-[400px] bg-gray-50 dark:bg-gray-800/40 rounded-[1.8rem] p-3 border border-gray-100 dark:border-gray-800/60">
                  <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                    {(grouped[status] ?? []).map(c => (
                      <VisaCard key={c.id} c={c} onClick={() => setSelected(c)} />
                    ))}
            ...
            {denied.length > 0 && (
              <div className="w-64 shrink-0 flex flex-col">
                <div className="flex items-center justify-between mb-3 px-1">
                  <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border bg-red-50 text-red-700 border-red-200">Denied</span>
                  <span className="text-[10px] text-gray-400 font-black">{denied.length}</span>
                </div>
                <div className="flex-1 flex flex-col h-[400px] bg-gray-50 dark:bg-gray-800/40 rounded-[1.8rem] p-3 border border-gray-100 dark:border-gray-800/60">
                  <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                    {denied.map(c => <VisaCard key={c.id} c={c} onClick={() => setSelected(c)} />)}
                  </div>
                </div>
              </div>
            )}
