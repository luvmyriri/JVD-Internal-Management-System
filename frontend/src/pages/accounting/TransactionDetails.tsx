    ...
    <main className="jvd mx-auto w-full max-w-[1550px] space-y-5 pb-12">
      ...
      <section id="documents" aria-labelledby="documents-title" className="scroll-mt-24 rounded-2xl border border-border bg-surface p-5">
        ...
        {canEditAccounting && <form onSubmit={(event) => { event.preventDefault(); if (!email.trim()) return toast.error('Enter a customer email address.'); emailMutation.mutate(); }} className="mt-6 flex flex-col gap-3 border-t border-border pt-5 sm:flex-row sm:items-end">
          ...
          <button type="submit" disabled={emailMutation.isPending || ['queued', 'sending'].includes(transaction.document_delivery?.status ?? '')} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#071b33] px-4 text-xs font-black text-white outline-none hover:bg-[#0d3159] disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2"><Mail className="h-4 w-4" />{emailMutation.isPending ? 'Queueing…' : ['queued', 'sending'].includes(transaction.document_delivery?.status ?? '') ? 'Sending…' : ['failed', 'stalled'].includes(transaction.document_delivery?.status ?? '') ? 'Retry invoice email' : 'Send invoice email'}</button>
        </form>}
      </section>
