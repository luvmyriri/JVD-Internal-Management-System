  ...
  const handleExcelExport = async () => {
    ...
    try {
      ...
      worksheet.spliceRows(1, 0,
        ['JVD EVENTS & TRAVEL MANAGEMENT CO.'],
        [`AGENT SALES LOG & PERFORMANCE METRICS`],
        [`Generated by: ${user?.first_name} ${user?.last_name || 'System Admin'}`],
        [`Date: ${new Date().toLocaleString()}`],
        [`Filter Period: ${range.toUpperCase()}`],
        [`Total Sales Covered: PHP ${kpis.totalSales.toLocaleString()}`],
        []
      );
          ...
          <div className="flex overflow-x-auto hide-scrollbar flex-nowrap w-full md:w-auto bg-gray-100/80 dark:bg-gray-800/80 p-1 rounded-xl border border-gray-200/50 dark:border-gray-700/50 shadow-inner backdrop-blur-md">
            {['day', 'week', 'month', 'year', 'all'].map((r) => (
              <button
                key={r}
                onClick={() => setRange(r)}
                className={`shrink-0 whitespace-nowrap px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${range === r
              ...
              <div className="flex overflow-x-auto hide-scrollbar flex-nowrap w-full sm:w-auto bg-gray-100/80 dark:bg-gray-800/60 p-0.5 rounded-lg border border-gray-200/20 dark:border-gray-700/20">
                {['All', 'Paid', 'Partial', 'Pending', 'Cancelled'].map((status) => (
                  <button
                    ...
                    className={`shrink-0 whitespace-nowrap px-2.5 py-1 rounded text-[8px] font-bold transition-all cursor-pointer ${statusFilter === status
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    {/* Rank Badge */}
                    <div className={`w-5 h-5 rounded-lg text-[9px] font-black flex items-center justify-center shrink-0 shadow-sm ${idx === 0
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 backdrop-blur-sm bg-slate-950/40 select-none">
          ...
          <div className="w-full max-w-lg bg-white dark:bg-gray-900 rounded-[2rem] p-6 border border-gray-100 dark:border-gray-850 shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-start justify-between mt-2 mb-4 shrink-0">
              <div>
                <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[8px] font-bold border mb-1.5 ${selectedTxn.status === 'Paid'
                    ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20'
                    : selectedTxn.status === 'Pending'
                      ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20'
                      : 'bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/20'
                  }`}>
                  <span className={`w-1 h-1 rounded-full ${selectedTxn.status === 'Paid' ? 'bg-emerald-500' : selectedTxn.status === 'Pending' ? 'bg-amber-500 animate-pulse' : 'bg-rose-500'
                    }`} />
                  {selectedTxn.status}
                </span>
