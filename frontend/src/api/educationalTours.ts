...
export interface EducationalProgram {
  id: number; name: string; learning_objectives?: string | null; default_stops: string[];
  minimum_students: number; students_per_chaperone: number; students_per_free_chaperone: number;
  student_price: number; additional_chaperone_price: number; includes_meals: boolean; includes_coordinator: boolean;
  includes_insurance: boolean; includes_shirt: boolean; created_at?: string; updated_at?: string;
}
...
export interface EducationalPricing {
  student_count: number; chaperone_count: number; required_chaperones: number; free_chaperone_count: number;
  chargeable_chaperone_count: number; student_amount: number; chaperone_amount: number; subtotal: number;
...
export interface EducationalBooking {
  id: number; reference: string; school_name: string; grade_level: string; starts_at: string; ends_at: string;
  student_count: number; chaperone_count: number; status: string; program: EducationalProgram;
...
export const educationalTourApi = {
  ...
  quote: (programId: number, students: number, chaperones: number) => client.post('/sales/educational-quote', { program_id: programId, student_count: students, chaperone_count: chaperones }).then(res => res.data.data as EducationalPricing),
